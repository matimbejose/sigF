...
var signatureForm = (function () {
    ...
    var init = function (fe) {
        ...
        formElements.signButtonInf.click(signInf);
        formElements.signButtonLote.click(signLote);
...
//            $.unblockUI();
    ...
    var signLote = function () {

        var selectedCertThumbprint = formElements.certificateSelect.val();
        var signatureLoteRps = $("#toSignHashFieldLoteRps").val();

        pki.readCertificate(selectedCertThumbprint).success(function (certificate) {
            $(".certificateField").val(certificate);
            pki.signHash({
                thumbprint: selectedCertThumbprint,
                hash: signatureLoteRps,
                digestAlgorithm: 'SHA-1'
            }).success(function (signature) {
                $(".signatureFieldLoteRps").val(signature);
                rcLote();
            });
        });
    };
